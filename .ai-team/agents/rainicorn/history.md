...
📌 Role-Aware Note Selection implemented (issue #33) — by Rainicorn
Created `src/SqncR.Core/Generation/RoleNoteSelector.cs` with RoleOutput/RoleNote records and RoleNoteSelector class. Each InstrumentRole gets musically appropriate behavior: Bass plays root/5th in octave 2-3 (MIDI 36-59) with velocity 80-100 and quarter/half note rhythm, occasional walking bass passing tones. Pad plays 3-note chord voicings (root+3rd+5th) in octave 3-5 (MIDI 48-83), velocity 50-70, whole note duration, changes every 4-8 beats with chord caching. Lead/Melody uses pluggable INoteGenerator in octave 4-6 (MIDI 60-95), velocity 70-90, 8th to quarter note rhythm. Drums delegates to StepSequencer+DrumMap, velocity from pattern. Added NoteSelector property to GenerationState. 19 tests in `tests/SqncR.Core.Tests/Generation/RoleNoteSelectorTests.cs` covering all roles: pitch ranges, velocity ranges, scale membership, chord voicing count, duration, generator pluggability, drum map integration. All 526 tests pass, 0 warnings.

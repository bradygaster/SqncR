...
📌 Issue #35: Hardware telemetry (per-device latency, polyphony tracking) implemented
Created `InstrumentTelemetry.cs` (ActivitySource "SqncR.Instruments", Meter "SqncR.Instruments") with TraceNoteSent span (instrument.id, midi.channel, midi.note, midi.velocity tags), RecordPolyphony for per-instrument utilization tracking, RecordLatency histogram, RecordError counter. 4 metric instruments: instrument.notes_sent (Counter), instrument.polyphony_utilization (ObservableGauge, tagged by instrument.id), instrument.latency_ms (Histogram), instrument.errors (Counter). Created `PerInstrumentNoteTracker.cs` — thread-safe per-instrument note tracking with NoteOn, NoteOff, GetActiveCount, AllNotesOff partitioned by instrument ID. Extended PlannedNote with optional InstrumentId. Updated ChannelRouter to pass instrument IDs through PlannedNote. Integrated into GenerationEngine.EmitPlannedNote: calls TraceNoteSent, tracks per-instrument polyphony via PerInstrumentNoteTracker, records errors on failure. Updated McpServer Program.cs with "SqncR.Instruments" ActivitySource and Meter registration. Created 14 InstrumentTelemetryTests covering span creation/tags, counter increments, polyphony tracking, multi-instrument isolation, latency histogram, error counter, PerInstrumentNoteTracker isolation/cleanup. Build: 0 errors, 0 warnings. Tests: 576 passing.
